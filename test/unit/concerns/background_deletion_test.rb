...
module Concerns
  class BackgroundDeletionTest < ActiveSupport::TestCase
    ...
    def test_destroy_integration
      ...
      DeleteObjectHierarchyWorker.expects(:perform_later).with(user, anything, {options: {background_destroy_method: 'destroy', lock: false}}).once
      DeleteObjectHierarchyWorker.expects(:perform_later).with(user, anything, {options: {background_destroy_method: 'delete', lock: false}}).never
    ...
    def test_delete_integration
      ...
      DeleteObjectHierarchyWorker.expects(:perform_later).with(user, anything, {options: {background_destroy_method: 'delete', lock: false}}).once
      DeleteObjectHierarchyWorker.expects(:perform_later).with(user, anything, {options: {background_destroy_method: 'destroy', lock: false}}).never
      DeleteObjectHierarchyWorker.perform_now(double_object)
    end
